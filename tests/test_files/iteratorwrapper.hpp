...
// than the Value type in its type. For multiindex this probably means every instance, because you are also specifying
// the members which make up the index in the template arguments.
...
struct CrazyBoostIteratorWrapper: public IteratorWrapper<typename std::set<Value>::iterator, Value> //Note: here the set could be replaced with any weird boost container
{
  ...
  //get() is inherited and we can still wrap it in python (without specifically declaring the inheritance there). See pxd.
};